...
const sWhere = '__tests__/lib/utils.test.js'
...
describe('./src/lib/utils', () => {
    describe('objectToQueryString => queryStringToObject', () => {
        ...
        it('converts...', () => {
            const sWho = '${sWhere}::converts'

            test_cases.forEach(({ input, expected_output, linda_round_trippable }) => {
                const actual_output = utils.objectToQueryString(input)
                ...
                if (linda_round_trippable) {
                    // round trip...
                    const round_trip_object = utils.queryStringToObject(actual_output)
    ...
    describe('strContainsIgnoreCase', () => {
        ...
        const { strContainsIgnoreCase } = utils

        testCases.forEach((testCase) => {
            const { strFinder, strFindIn, expectedOutput } = testCase
            const label = `strContainsIgnoreCase('${strFinder}','${strFindIn}') returns ${expectedOutput}...`
            it(label, () => {
    ...
    describe('strBeginsWithIgnoreCase', () => {
        ...
        const { strBeginsWithIgnoreCase } = utils

        testCases.forEach((testCase) => {
            const { strFinder, strFindIn, expectedOutput } = testCase
            const label = `strBeginsWithIgnoreCase('${strFinder}','${strFindIn}') returns ${expectedOutput}...`
            it(label, () => {
    ...
    describe('compareStrings', () => {
        ...
        const { compareStrings } = utils

        testCases.forEach((testCase) => {
            const { strA, strB, expectedOutput } = testCase
            const label = `compareStrings('${strA}','${strB}') returns ${expectedOutput}...`
            it(label, () => {
    ...
    describe('strEqualsIgnoreCase', () => {
        ...
        const { strEqualsIgnoreCase } = utils

        testCases.forEach((testCase) => {
            const { strA, strB, expectedOutput } = testCase
            const label = `strEqualsIgnoreCase('${strA}','${strB}') returns ${expectedOutput}...`
            it(label, () => {
