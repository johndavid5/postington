...
jest.mock('../../src/server/models/posts', () => require('../../src/server/models/__mocks__/posts'))

describe('posts_api...', () => {
    let mockPosts = {}
    let numPosts = -1
    const mockPostsModel = new PostsMockModel()
    ...
    test('doGet()...', (done) => {
        ...
        const request_mock = new RequestMock()
        request_mock.setQuery('title.contains', 'est')
        const response_mock = new ResponseMock()
        ...
        doGet(request_mock, response_mock, {
            callback: () => {
                ...
                const payload = response_mock.json.mock.calls[0][0]
