...
import {
    postsFetch, postsIsFetching, postsLocalFilter, postEditStart, postEditCancel, postEditFinish,
} from '../src/actions'
...
const sWhere = './__tests__/actions.js'
...
describe('Action Creators', () => {
    ...
    describe('postsFetch() thunk-based action creator...', () => {
        ...
        const sWhich = `${sWhere}: postsFetch() thunk-based action creator`

        beforeAll((done) => {
            const sWho = `${sWhich}: beforeAll`
            ...
            const le_filters = { description_filter: 'Macbeth' }
            ...
            const postsFetchThunk = postsFetch(le_filters) // Thunk returns a function...
        ...
        it('should have posts fetched from the remote site', () => {
            const sWho = `${sWhich}: should have posts fetched from the remote site`
        ...
        it('postsLocalFilter() action filters properly from remote_posts_list into posts_list...', (done) => {
            const sWho = `${sWhich}: postsLocalFilter() action filters properly from remote_posts_list into posts_list...`

            const leFiltersAlpha = {
                body_filter: 'a',
            ...
            const leFilters = { user_id_filter: 1, sort_by_field: 'title', sort_by_asc_desc: 'asc' }

            const postsLocalFilterThunk = postsLocalFilter(leFilters) // Thunk returns a function...
    ...
    describe('postsIsFetching()...client-based action...(it uses thunks immediately dispatched by the client...)', () => {
        ...
        beforeAll(() => {
            const sWho = '__tests__/actions.js: postsIsFetching():beforeAll'
            logajohn.debug(`${sWho}(): SHEMP: Moe, storeFactory = `, storeFactory)
            const bServer = true
        ...
        it('can dispatch POSTS_FETCHING true', () => {
            const sWho = '__tests__/actions.js: can dispatch POSTS_FETCHING true'
        ...
        it('can dispatch POSTS_FETCHING false', () => {
            const sWho = '__tests__/actions.js: can dispatch POSTS_FETCHING false'
    ...
    describe('postsLocalFilter()...', () => {
        ...
        const sWhich = `${sWhere}: postsLocalFilter()`
        ...
        it('filters properly from remote_posts_list into posts_list...', (done) => {
            const sWho = `${sWhich}: filters from remote_posts_list into posts_list...`
    ...
    describe('postEditStart()...', () => {
        ...
        const sWhich = `${sWhere}: postEditStart()`

        const postIsEditingId = 3

        const iWhere = postsFixture.array.findIndex(post => post.id == postIsEditingId)
        ...
        beforeAll((done) => {
            ...
            const postEditStartThunk = postEditStart(postIsEditingId) // Thunk returns a function...
    ...
    describe('postEditCancel()...', () => {
        ...
        const sWhich = `${sWhere}: postEditCancel()`
    ...
    describe('postEditFinish()...', () => {
        ...
        const sWhich = `${sWhere}: postEditFinish()`
        ...
        const postIsEditingId = editedPost.id

        const expectedNewRemotePostsList = [...postsFixture.array]

        const iWhere = expectedNewRemotePostsList.findIndex(post => post.id == postIsEditingId)
        ...
        beforeAll((done) => {
            ...
            const postEditFinishThunk = postEditFinish(postIsEditingId, editedPost) // Thunk returns a function...
            logajohn.debug(`${sWho}(): SHEMP: Moe, dispatchin' postEditStartThunk()...`)
