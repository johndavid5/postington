...
export const postsFetch = function postsFetcherator(filters){
  ...
  return new Promise(function(resolve, reject) {
    ...
    let mixin = { timestamp: new Date().toString() }
...
export const postsLocalFilter = function postsLocalFilterator(filters){
  ...
  return function postsLocalFilterThunk(dispatch,getState){
	  ...
	  return new Promise(function(resolve, reject) {
	    ...
	    let { remote_posts_list } = getState().posts
	
	    let postsModel = new Posts()
	
	    let postsModelSrc = {'array': remote_posts_list }
		...
		.then((numPosts)=>{})
	    .then(()=>{
	        return postsModel.getPosts(filters)
	    })
	    .then((posts) => {
...
export const postEditStart = function postEditStartThunkerator(isEditingId){
    ...
    return function postEditStartThunk(dispatch,getState){
	  ...
	  return new Promise(function(resolve, reject) {
		  ...
		  let { posts_list } = getState().posts 
		
		  let iWhich = posts_list.findIndex((post)=>{ return post.id == isEditingId })
		
		  if( iWhich > -1 ){
		    let oWhich = {...posts_list[iWhich]}
